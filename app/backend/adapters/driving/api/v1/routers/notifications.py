...
from adapters.driving.api.v1.dependencies import NotificationRepositoryDep
...
async def list_notifications(
    ...
    items = notification_repository.list_for_user_sync(current_user.user_id, limit=limit)
    unread_count = notification_repository.count_unread_for_user_sync(current_user.user_id)
...
async def mark_notification_read(
    ...
    notification_repository.mark_read_sync(notification_id, current_user.user_id)
    return {"ok": True}
...
async def mark_all_notifications_read(
    ...
    count = notification_repository.mark_all_read_sync(current_user.user_id)
